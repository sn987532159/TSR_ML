...
column_names = ["1-month mRS", "Height", "Weight", "GCS-E", "GCS-V", "GCS-M", "Systolic BP", "Diastolic BP",
                ...
                "Cancer", "Atrial fibrillation", "Heart failure", "Ischemic heart (CAD or old MI)",
                "AMI smaller than 4W",
...
## input dataset
tsr_train_x = tsr_train.drop(["icase_id", "idcase_id", "mrs_tx_3", "CHANGE"], axis=1).reset_index(drop=True)
print(tsr_train_x.shape)

tsr_validation_x = tsr_validation.drop(["icase_id", "idcase_id", "mrs_tx_3", "CHANGE"], axis=1).reset_index(drop=True)
print(tsr_validation_x.shape)

tsr_test_x = tsr_test.drop(["icase_id", "idcase_id", "mrs_tx_3", "CHANGE"], axis=1).reset_index(drop=True)
...
csv_save = os.path.join("..", "data", "LINKED_DATA", "TSR_ALL", "TSR_ALL31", "TSR_ALL31_y_TRAIN.csv")
pd.DataFrame(tsr_train_y).to_csv(csv_save, index=False)

csv_save = os.path.join("..", "data", "LINKED_DATA", "TSR_ALL", "TSR_ALL31", "TSR_ALL31_y_VALIDATION.csv")
pd.DataFrame(tsr_validation_y).to_csv(csv_save, index=False)

csv_save = os.path.join("..", "data", "LINKED_DATA", "TSR_ALL", "TSR_ALL31", "TSR_ALL31_y_TEST.csv")
...
ohe.get_feature_names(nominal_features)
tsr_train_x = tsr_train_x.drop(nominal_features, axis=1)
tsr_train_x.columns = column_names

csv_save = os.path.join("..", "data", "LINKED_DATA", "TSR_ALL", "TSR_ALL31", "TSR_ALL31_X_TRAIN.csv")
...
csv_save = os.path.join("..", "data", "LINKED_DATA", "TSR_ALL", "TSR_ALL31", "TSR_ALL31_X_VALIDATION.csv")
...
csv_save = os.path.join("..", "data", "LINKED_DATA", "TSR_ALL", "TSR_ALL31", "TSR_ALL31_X_TEST.csv")
