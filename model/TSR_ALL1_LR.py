...
joblib.dump(lrG, "model_pickle/MICE1/TSR_ALL1G_LR_BASED.pkl")

### TUNED LR
lrG_gsCV = lrG_gscv.fit(G_X_train, G_y_train)
joblib.dump(lrG_gsCV, "model_pickle/MICE1/TSR_ALL1G_LR_TUNED.pkl")

### CALIBRATED LR
lrG_cccv = CalibratedClassifierCV(base_estimator=lrG_gsCV.best_estimator_, cv=5)
lrG_ccCV = lrG_cccv.fit(G_X_train, G_y_train)
joblib.dump(lrG_ccCV, "model_pickle/MICE1/TSR_ALL1G_LR_CALIBRATED.pkl")

#BAD when discharged
### BASED LR
lrB = lr.fit(B_X_train, B_y_train)
joblib.dump(lrB, "model_pickle/MICE1/TSR_ALL1B_LR_BASED.pkl")

### TUNED LR
lrB_gsCV = lrB_gscv.fit(B_X_train, B_y_train)
joblib.dump(lrB_gsCV, "model_pickle/MICE1/TSR_ALL1B_LR_TUNED.pkl")

### CALIBRATED LR
lrB_cccv = CalibratedClassifierCV(base_estimator=lrB_gsCV.best_estimator_, cv=5)
lrB_ccCV = lrB_cccv.fit(B_X_train, B_y_train)
joblib.dump(lrB_ccCV, "model_pickle/MICE1/TSR_ALL1B_LR_CALIBRATED.pkl")