...
if __name__ == '__main__':
    ...
    csv_path = os.path.join("INFO", "TSR_ALL1", "Threshold_Intersection_1.csv")
    ...
    G_et_list = intersected_auc(G_X_train, G_X_test, G_y_train, G_y_test, T_I, "G1", based_et, G_ET_TUNED)

    ### XGBC
    pkl_path = os.path.join("..", "..", "model", "model_pickle", "TSR_ALL1G_XGBC_TUNED.pkl")
    G_XGBC_TUNED = joblib.load(pkl_path)
    G_xgbc_list = intersected_auc(G_X_train, G_X_test, G_y_train, G_y_test, T_I, "G1", based_xgbc, G_XGBC_TUNED)
    ...
    plt.title('TSR_ALL1_G_Intersection', fontsize=15)
    # plt.savefig('PLOT/TSR_ALL1/TSR_ALL1_G_Intersection.png')
    plt.show()
    ...
    B_et_list = intersected_auc(B_X_train, B_X_test, B_y_train, B_y_test, T_I, "B1", based_et, B_ET_TUNED)

    ### XGBC
    pkl_path = os.path.join("..", "..", "model", "model_pickle", "TSR_ALL1B_XGBC_TUNED.pkl")
    B_XGBC_TUNED = joblib.load(pkl_path)
    B_xgbc_list = intersected_auc(B_X_train, B_X_test, B_y_train, B_y_test, T_I, "B1", based_xgbc, B_XGBC_TUNED)
    ...
    plt.title('TSR_ALL1_B_Intersection', fontsize=15)
    # plt.savefig('PLOT/TSR_ALL1/TSR_ALL1_B_Intersection.png')
    plt.show()